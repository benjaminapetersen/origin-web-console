...
angular.module('openshiftConsole')
  .controller('CreateFromURLController', function ($scope, $routeParams, $location, $filter, AuthService, DataService, AlertMessageService, Navigate, ProjectsService ) {
    ...
    var alertInvalidApp = function(app) {
      $scope.alerts.invalidImageStream = {
        type: "error",
        message: "The app name \"" + app + "\" is not valid.  An app name is an alphanumeric (a-z, and 0-9) string with a maximum length of 24 characters, where the first character is a letter (a-z), and the '-' character is allowed anywhere except the first or last character."
      };
    };
    ...
    var validateApp = function (app) {
      return app.length < 25 && /^[a-z]([-a-z0-9]*[a-z0-9])?$/.test(app);
    };
    ...
    if (!(_.includes(namespaceWhitelist, createDetails.namespace))) {
      ...
      } else if (createDetails.app && !(validateApp(createDetails.app))) {
        alertInvalidApp(createDetails.app);
      } else {
        getResources();
      }
    }
