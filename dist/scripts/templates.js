angular.module('openshiftConsoleTemplates', []).run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('views/directives/key-value-editor.html',
    ...
    "<span ng-if=\"entry.isReadonlyValue\">\n" +
    "{{entry.valueFrom.configMapKeyRef.name}}\n" +
    "</span>\n" +
    "<a ng-if=\"!(entry.isReadonlyValue)\" ng-href=\"{{entry.apiObj | navigateResourceURL}}\">\n" +
    "{{entry.valueFrom.configMapKeyRef.name}}\n" +
    "</a>\n" +
    "</span>\n" +
    "<span ng-switch-when=\"secretKeyRef\">\n" +
    ...
    "<span ng-if=\"valueFromSelectorOptions && canGetSecretsAndConfigMaps\" class=\"action-divider\" aria-hidden=\"true\"> | </span>\n" +
    "<a ng-if=\"valueFromSelectorOptions && canGetSecretsAndConfigMaps\" href=\"\" class=\"add-row-link\" role=\"button\" aria-label=\"Add row\" ng-click=\"onAddRowWithSelectors()\">{{ addRowWithSelectorsLink }}</a>\n" +
    "</div>\n" +
    "</div>\n" +
    "</ng-form>"
  );
